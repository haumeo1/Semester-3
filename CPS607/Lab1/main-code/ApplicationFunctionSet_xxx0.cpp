...
static bool ApplicationFunctionSet_SmartRobotCarLeaveTheGround(void)
{
  ...
  else if(AppITR20001.DeviceDriverSet_ITR20001_getAnaloguexxx_M() > Application_FunctionSet.TrackingDetection_V)
  {
    Application_FunctionSet.Car_LeaveTheGround= false;
    return false;
  }
  else if(AppITR20001.DeviceDriverSet_ITR20001_getAnaloguexxx_L() > Application_FunctionSet.TrackingDetection_V)
  {
    Application_FunctionSet.Car_LeaveTheGround= false;
    return false;
  }
...
//        uint8_t switc_ctrl = 0;
        ...
        if (first_is == true) 
        {
            ...
            if (function_xxx(get_Distance, 0, 20)){
              ...
              ApplicationFunctionSet_SmartRobotCarMotionControl(stop_it, 0);
              delay_xxx(500);
              ApplicationFunctionSet_SmartRobotCarMotionControl(Forward, 40);
...
if (Car_LeaveTheGround == false){
   ApplicationFunctionSet_SmartRobotCarMotionControl(stop_it, 0);
   delay_xxx(500);
   ApplicationFunctionSet_SmartRobotCarMotionControl(Backward, 40);
   for (int angle = 30; angle <=160 ; angle+=120){
    AppServo.DeviceDriverSet_Servo_control(angle /* Position_angle */);
    AppULTRASONIC.DeviceDriverSet_ULTRASONIC_Get(&get_Distance /*out*/);
    if (function_xxx(get_Distance, 0, 7)){
        if(angle == 30){
  ...
  ApplicationFunctionSet_SmartRobotCarMotionControl(Forward, 30); // forward
  }
    }}
